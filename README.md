...
Rooms carry no resident resources; their only lifecycle rule is a lazy sweep. A room is two passive files under `<state dir>/rooms/`: `<long-id>.jsonl` (append-only sealed transcript of `created`/`join`/`leave`/`chat`/`closed` events) and `<long-id>.members.json` (the sealed member list). The 4-char id is the first 4 characters of the 16-char long id and is the join credential; there is deliberately no further access control — the shared state directory is one trust domain. Membership binds to Agent IDs: offline members are pruned lazily on the next room access and a restarted session must rejoin with the room id. Every `find_coagents()` call first sweeps empty rooms: the first observation of a room with no online members stamps `empty_since` (the room stays listed and rejoinable), and once that stamp is older than 2 hours the members file is deleted and the sealed transcript is moved — never deleted — to `<state dir>/archive/rooms/<long-id>.jsonl` with a final `closed` event. The 4-char id is then free for reuse; joining it afterwards fails with a not-found error. Rejoining an empty room clears the stamp, restarting the 2-hour grace.
...
- `list_agents()`
- `find_coagents()` — read-only Markdown table of online agents (copyable Agent ID, display name, repo, client, model, PID) plus, when rooms exist, a `Room ID / Members / Created` table; `#<room id>` is directly usable as `send(to=...)`. The call also sweeps rooms that have had no online members for over 2 hours (their transcripts move to `archive/rooms/`)
